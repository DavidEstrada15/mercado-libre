...
function Offerts() {
  ...
  return (<>
    ...
    <section className='bg-white py-10 text-center'>
      <h2 className='text-3xl capitalize  py-5 font-bold'>Ofertas</h2>
      <p>¡Encuentra precios increíbles cada día!</p>
    </section>
        ...
        <section className='grid grid-cols-1 bg-white lg:bg-transparent lg:grid-cols-4  gap-5 place-self-center h-full p-5'>
          {
            productos.map(producto => (
              <Link to={"/Productsingle"} key={producto.id} state={{producto}}> 
              <article className='flex w-full h-full lg:flex-col gap-5  bg-white '>
               <div className='sm:w-[40vw]  lg:w-[20vw] object-cover lg:h-[30vh] place-items-center ' > 
              <img className='w-[80%] h-full' src={producto.imagenes} alt={producto.imagenes} /></div>
                <div className='flex flex-col gap-10 bg-white'>
                  <h2 className='text-4xl capitalize'>{producto.nombre}</h2>
